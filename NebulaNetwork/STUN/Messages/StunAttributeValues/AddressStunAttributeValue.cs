...
public abstract class AddressStunAttributeValue : IStunAttributeValue
{
	...
	public virtual int WriteTo(Span<byte> buffer)
	{
		...
		BinaryPrimitives.WriteUInt16BigEndian(/*buffer[2..]*/buffer.Slice(2), Port);
		Requires.Range(Address.TryWriteBytes(/*buffer[4..]*/buffer.Slice(4), out int bytesWritten), nameof(buffer));
